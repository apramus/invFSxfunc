setwd("~/Desktop/invFSxfunc-master")
# 
# Supplementary Code for Ramus et al. (2017) An invasive foundation species enhances multifunctionality in a coastal ecosystem. PNAS 114(32):8580-8585.
...
# fit nls and generate model selection table
...
# R code to fit candidate models and generate 
# generate the 
...
# This code 
# respository contains the plot-level data used to generate all analyses and figures presented in the paper. 
# fit nonlinear models and 
# Code developed by Aaron Ramus (aaron.ramus@gmail.com). Last updated 20 Mar 2018
...
# Create function modfit to extract information and parameters data frame to fit models
# Function to return information about the fitted nls model
...
# Create parameter dataframes to fit nls models
...
# Define an object to stuff model selection tables into later
cum.nlsTab <- NULL

# Define an object to remove information?????
...
Hyp <- nls2(HypFormula, data=xy, start=c(a=75, b=1), nls.control(maxiter=200))  # can 
...
HypStart <- nls2(HypFormula, data=xy, start=ab, algorithm="brute-force")
Hyp <- nls2(HypFormula, data=xy, start=HypStart)
#Hyp <- nls2(HypFormula, data=xy, start=c(a=10, b=-0.5), nls.control(maxiter=200))
pHyp <- stat_function(fun=function(x){coef(Hyp)[1]*x/(coef(Hyp)[2]+x)}, aes(col="Hyperbolic"))
p+pHyp
...
responseName <- "Number of functions >= 10 % threshold"
...
responseName <- "Number of functions >= 20 % threshold"
...
responseName <- "Number of functions >= 30 % threshold"
...
responseName <- "Number of functions >= 40 % threshold"
...
responseName <- "Number of functions >= 50 % threshold"
...
responseName <- "Number of functions >= 60 % threshold"
...
responseName <- "Number of functions >= 70 % threshold"
...
responseName <- "Number of functions >= 80 % threshold"
...
responseName <- "Number of functions >= 90 % threshold"
...
write.csv(cum.nlsTab, "model selection table.csv")

##########################################################################################################################################

# Melt and organize data for plotting
...
ggsave(fig2, file="figure 2.png", width=4.5, height=5.1, dpi=1200)